...
直接对接支付宝开放平台。移动端默认走支付宝手机网站支付跳转；开启“支付宝移动端当面付唤起”后改为调用当面付，前端尝试打开支付宝 App，失败时显示动态二维码备用页。桌面端使用当面付 `alipay.trade.precreate` 返回原生扫码串并由前端渲染二维码；若商户未开通当面付或接口返回失败，系统会直接报错，不会降级成电脑网站支付收银台链接。
...
用户选择充值金额和支付方式
  ...
  用户完成支付
  ├─ EasyPay    → 扫码 / H5 跳转
  ├─ 支付宝官方  → 桌面当面付扫码单 / 移动端 WAP 或当面付唤起 + 动态二维码备用页
