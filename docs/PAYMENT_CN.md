...
| **支付宝官方** | 桌面 / H5 当面付二维码扫码 | 直接对接支付宝开放平台，统一返回当面付二维码 |
...
直接对接支付宝开放平台。桌面端和移动 H5 端统一使用当面付 `alipay.trade.precreate` 返回原生扫码串，前端渲染二维码；手机端用户可长按保存二维码后在支付宝中识别付款。若商户未开通当面付或接口返回失败，系统会直接报错，不会降级成手机网站支付或电脑网站支付收银台链接。
