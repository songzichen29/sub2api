...
// Any API-key account may point to a compatible upstream that exposes billing metadata.
const eligible = computed(() => props.account.type === 'apikey')
const snapshot = computed<UpstreamBillingProbeSnapshot | undefined>(
  () => props.account.extra?.upstream_billing_probe as UpstreamBillingProbeSnapshot | undefined,
)
