<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <AccountTagsFilter
      :model-value="Array.isArray(filters.tags) ? filters.tags : []"
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; availableTags?: string[] }>()
...
const updateTags = (value: string[]) => { emit('update:filters', { ...props.filters, tags: value }) }
...
const sOpts = computed(() => [{ value: '', label: t('admin.accounts.allStatus') }, { value: 'active', label: t('admin.accounts.status.active') }, { value: 'unschedulable', label: t('admin.accounts.status.paused') }, { value: 'inactive', label: t('admin.accounts.status.inactive') }, { value: 'error', label: t('admin.accounts.status.error') }, { value: 'rate_limited', label: t('admin.accounts.status.rateLimited') }, { value: 'temp_unschedulable', label: t('admin.accounts.status.tempUnschedulable') }])
