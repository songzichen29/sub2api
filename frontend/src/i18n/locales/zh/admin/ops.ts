export default {
  ops: {
    ...
    errorIngestion: {
      title: '错误采集与入口拒绝',
      description: '区分错误已入队、已处理和已落库，并展示认证或准入阶段未进入普通错误表的拒绝请求。',
      refresh: '刷新采集状态',
      errorPipeline: '普通错误日志采集',
      rejectPipeline: '入口拒绝采集',
      state: {
        healthy: '正常',
        warning: '存在异常',
        stopped: '已停止',
      ...
      flushed: '已落库请求',
      overflowed: '维度溢出',
      recentRejects: '最近入口拒绝',
      total: '共 {count} 项',
      emptyRejects: '当前时间窗口内没有入口拒绝。',
      reason: '拒绝原因',
      route: '路由 / 协议',
      identity: '用户 / API Key',
      count: '请求数',
      lastSeen: '最近发生',
      errorHealth: '普通错误采集状态',
      rejectHealth: '入口拒绝采集状态',
      rejectList: '入口拒绝列表',
      loadFailed: '以下监控数据加载失败：{targets}',
      reasons: {
        query_api_key_deprecated: '禁止通过查询参数传递 API Key',
        api_key_required: '缺少 API Key',
        invalid_api_key: 'API Key 无效',
        invalid_auth_rate_limited: '无效认证触发限流',
        api_key_auth_overloaded: 'API Key 认证过载',
        api_key_disabled: 'API Key 已禁用',
        ip_restricted: 'IP 不在允许范围',
        user_inactive: '用户已停用',
        group_deleted: '分组已删除',
        group_disabled: '分组已禁用',
        group_not_allowed: '分组无访问权限',
        group_unassigned: 'API Key 未分配分组',
        other: '其他入口拒绝'
      }
    },
    requestsTotal: '请求（总计）',
    ...
    tooltips: {
      ...
      errorDistribution: '按状态码统计全部错误分布，包含业务限制；可进入明细查看完整记录。',
