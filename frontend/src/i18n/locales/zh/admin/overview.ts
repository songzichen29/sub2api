export default {
  ...
  backup: {
    ...
    imageStorage: {
      title: '图片对象存储',
      description: '配置用于异步生成图片的 S3 兼容存储。',
      enabled: '启用图片存储',
      reuseBackupS3: '复用数据库备份 S3 配置',
      bucket: '存储桶',
      bucketInherited: '继承数据库备份 S3 配置',
      prefix: 'Key 前缀',
      publicBaseUrl: '公开基础 URL',
      publicBaseUrlPlaceholder: 'https://cdn.example.com',
      presignExpiryHours: '预签名 URL 有效期（小时）',
      saved: '图片存储配置已保存'
    },
    schedule: {
      title: '定时备份',
  ...
  users: {
    ...
    bulkLimits: {
      ...
      enableConcurrency: '启用并发数更新',
      enableRPMLimit: '启用 RPM 限制更新',
      unlimited: '0 表示不限制',
      nonNegativeInteger: '数值必须是大于等于 0 的整数。',
      selectionLimit: '最多选择 {max} 个用户。',
      applying: '应用中...',
      apply: '应用修改',
      concurrencyValue: '并发数：{value}',
      rpmUnlimitedValue: 'RPM：不限制',
      rpmValue: 'RPM：{value}',
      confirm: '确定要将 {fields} 应用到已选择的 {count} 个用户吗？',
      success: '已更新 {count} 个用户的限额',
      failed: '更新用户限额失败'
    },
    deposit: '充值',
    withdraw: '退款',
  ...
  groups: {
    ...
    form: {
      ...
      statusLabel: '状态',
      maxReasoningEffort: '最大推理强度',
      maxReasoningEffortHint: '超过该推理强度的请求将被限制为此值。',
      maxReasoningEffortUnlimited: '不限制',
      reasoningEffortMappings: '推理强度映射',
      addReasoningEffortMapping: '添加映射',
      reasoningEffortFrom: '来源',
      reasoningEffortFromPlaceholder: '选择来源强度',
      reasoningEffortTo: '目标',
      reasoningEffortToPlaceholder: '选择目标强度',
      removeReasoningEffortMapping: '删除映射',
      fromRequired: '请选择来源强度。',
      toRequired: '请选择目标强度。',
      duplicateFrom: '每个来源强度只能映射一次。',
      unsupportedFrom: '所选平台不支持该来源强度。',
      unsupportedTo: '所选平台不支持该目标强度。'
    },
    exclusiveObj: {
      yes: '是',
      no: '否'
