...
const orderTypeLabel = computed(() => {
  const currentOrder = hasDetailedOrder(order.value) ? order.value : null
  if (currentOrder?.order_type === 'subscription') return t('payment.admin.subscriptionOrder')
  if (currentOrder?.order_type === 'daily_limit_reset') return t('payment.admin.dailyLimitResetOrder')
...
const orderContentLabel = computed(() => {
  ...
  if (currentOrder.order_type === 'subscription') {
    const name = currentOrder.product_name || currentOrder.group_name
    const fixedExpiresAt = formatFixedSubscriptionExpiresAt(currentOrder.subscription_plan_expires_at)
    ...
    if (name && currentOrder.subscription_days) return `${name} · ${currentOrder.subscription_days}${t('payment.admin.days')}`
    if (name) return name
    if (currentOrder.subscription_days) return `${currentOrder.subscription_days}${t('payment.admin.days')}`
    return ''
  }
  if (currentOrder.order_type === 'daily_limit_reset') {
    return currentOrder.group_name || t('payment.admin.dailyLimitResetOrder')
  }
  return ''
})
...
const statusTitle = computed(() => {
  if (isSuccess.value) {
    const currentOrder = hasDetailedOrder(order.value) ? order.value : null
    if (currentOrder?.order_type === 'subscription') return t('payment.result.subscriptionSuccess')
    if (currentOrder?.order_type === 'daily_limit_reset') return t('payment.result.dailyResetSuccess')
...
function hasDetailedOrder(nextOrder: ResolvedOrder | null): nextOrder is PaymentOrder {
  return hasAmountFields(nextOrder)
}
