...
const menuItem = computed(() => {
  ...
  const publicItems = (appStore.cachedPublicSettings?.custom_menu_items ?? []) as CustomMenuItem[]
  const found = publicItems.find((item) => item.id === id) ?? null
  if (found) return found
  if (authStore.isAdmin) {
    return (adminSettingsStore.customMenuItems as CustomMenuItem[]).find((item) => item.id === id) ?? null
  }
  return null
})
