...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.type'), t('usage.pricingTier'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.inbound_endpoint || '', log.upstream_endpoint || '', getRequestTypeLabel(log), log.billing_tier || '',
...
const allColumns = computed(() => [
  ...
  { key: 'pricing_tier', label: t('usage.pricingTier'), sortable: false },
