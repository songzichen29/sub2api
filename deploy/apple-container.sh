...
MYSQL_CONTAINER="sub2api-apple-mysql"
REDIS_CONTAINER="sub2api-apple-redis"
APP_VOLUME="sub2api-apple-data"
MYSQL_VOLUME="sub2api-apple-mysql-data"
...
Commands:
  ...
  logs <service> [-f]   Show logs for app, mysql, or redis
...
preflight_stack_ownership() {
    ...
    for resource_name in "${APP_CONTAINER}" "${REDIS_CONTAINER}" "${MYSQL_CONTAINER}"; do
    ...
    for resource_name in "${APP_VOLUME}" "${REDIS_VOLUME}" "${MYSQL_VOLUME}"; do
...
cmd_init() {
    local env_dir temp_file database_secret jwt_secret totp_secret
    ...
    database_secret="$(generate_secret)" || die "Failed to generate MySQL password."
    ...
    [[ -n "${database_secret}" && -n "${jwt_secret}" && -n "${totp_secret}" ]] || \
    ...
    chmod 600 "${temp_file}"
    replace_env_value DATABASE_PASSWORD "${database_secret}" "${temp_file}"
...
prepare_environment() {
    ...
    if [[ -z "${APP_IMAGE}" ]]; then
        APP_IMAGE="$(read_env_value SUB2API_IMAGE_NAME)"
    fi
    MYSQL_IMAGE="$(read_env_value APPLE_CONTAINER_MYSQL_IMAGE mysql:8.0)"
    ...
    DATABASE_USER="$(read_env_value DATABASE_USER sub2api)"
    ...
    [[ -n "${DATABASE_USER}" ]] || die "DATABASE_USER must not be empty."
    [[ -n "${DATABASE_DBNAME}" ]] || die "DATABASE_DBNAME must not be empty."
    if [[ -z "${DATABASE_PASSWORD}" || "${DATABASE_PASSWORD}" == "change_this_secure_password" ]]; then
        die "Set a secure DATABASE_PASSWORD in ${ENV_FILE}."
    fi

    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sub2api-apple.XXXXXX")"
    APP_ENV_FILE="${TEMP_DIR}/app.env"
    MYSQL_ENV_FILE="${TEMP_DIR}/mysql.env"
    MYSQL_PROBE_ENV_FILE="${TEMP_DIR}/mysql-probe.env"
...
EOF
    ...
    chmod 600 "${MYSQL_ENV_FILE}" "${MYSQL_PROBE_ENV_FILE}" "${REDIS_ENV_FILE}"
}

prepare_app_environment() {
    [[ -n "${MYSQL_ADDRESS}" && -n "${REDIS_ADDRESS}" ]] || \
...
probe_mysql() {
    container exec --env-file "${MYSQL_PROBE_ENV_FILE}" \
        "${MYSQL_CONTAINER}" \
        mysqladmin ping --protocol=tcp --host=127.0.0.1 \
        --user="${DATABASE_USER}" --silent
}
...
start_dependencies() {
    start_container_if_needed "${MYSQL_CONTAINER}"
    if ! wait_for_probe "MySQL" 90 probe_mysql; then
        show_failure_logs "${MYSQL_CONTAINER}"
...
cmd_up() {
    ...
    ensure_image_available "${MYSQL_IMAGE}"
    ...
    if [[ "${recreate}" == true ]]; then
        ...
        delete_container_if_present "${MYSQL_CONTAINER}"
    fi

    ensure_container "${MYSQL_CONTAINER}" create_mysql_container
    ensure_container "${REDIS_CONTAINER}" create_redis_container
    start_dependencies
    MYSQL_ADDRESS="$(container_ipv4_address "${MYSQL_CONTAINER}")"
...
cmd_status() {
    ...
    print_container_status mysql "${MYSQL_CONTAINER}"
    print_container_status redis "${REDIS_CONTAINER}"

    if [[ -f "${ENV_FILE}" ]]; then
        prepare_environment
        if container_is_running "${MYSQL_CONTAINER}" && probe_mysql >/dev/null 2>&1; then
            printf '%-12s %s\n' "mysql" "healthy"
        else
            printf '%-12s %s\n' "mysql" "unhealthy"
            failed=1
...
cmd_logs() {
    ...
    case "${service}" in
        ...
        mysql) container_name="${MYSQL_CONTAINER}" ;;
        redis) container_name="${REDIS_CONTAINER}" ;;
        *) die "Unknown service '${service}'. Use app, mysql, or redis." ;;
    esac
...
cmd_pull() {
    ...
    info "Pulling ${MYSQL_IMAGE}..."
    container image pull --platform "${PLATFORM}" "${MYSQL_IMAGE}"
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        delete_volume_if_present "${MYSQL_VOLUME}"
