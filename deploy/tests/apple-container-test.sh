...
grep -q '^DATABASE_PASSWORD=change_this_secure_password$' "${ENV_FILE}" && fail "init retained the placeholder password"
...
assert_exists "${STATE_DIR}/containers/sub2api-apple-mysql"
...
assert_missing "${STATE_DIR}/running/sub2api-apple-mysql"
...
assert_missing "${STATE_DIR}/volumes/sub2api-apple-mysql-data"
