...
func (s *APIKeyRepoSuite) TestGetByKeyForAuth_PreservesDailyOverdraftFlag() {
	user := s.mustCreateUser("getbykey-auth-overdraft@test.com")
	dailyLimit := 120.0
	group, err := s.client.Group.Create().
		SetName("g-auth-overdraft").
		SetPlatform(service.PlatformOpenAI).
		SetStatus(service.StatusActive).
	...
	key := &service.APIKey{
		UserID:  user.ID,
		Key:     "sk-getbykey-auth-overdraft",
		Name:    "Overdraft Key",
		GroupID: &group.ID,
		Status:  service.StatusActive,
	}
	...
	s.Require().NoError(err)
	s.Require().NotNil(got.Group)
	s.Require().True(got.Group.AllowDailyOverdraft)
	s.Require().NotNil(got.Group.DailyLimitUSD)
	s.Require().Equal(dailyLimit, *got.Group.DailyLimitUSD)
}
