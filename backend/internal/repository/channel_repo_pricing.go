...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
			 SET models = ?, billing_mode = ?, input_price = ?, output_price = ?, cache_write_price = ?, cache_write_1h_price = ?, cache_read_price = ?, fast_multiplier = ?, flex_multiplier = ?, max_reasoning_effort_multiplier = ?, image_input_price = ?, image_output_price = ?, per_request_price = ?, time_pricing = ?, platform = ?, updated_at = NOW()
			 WHERE id = ?`,
...
func (r *channelRepository) batchLoadModelPricing(ctx context.Context, channelIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		fmt.Sprintf(`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, fast_multiplier, flex_multiplier, max_reasoning_effort_multiplier, image_input_price, image_output_price, per_request_price, time_pricing, created_at, updated_at FROM channel_model_pricing WHERE channel_id IN (%s) ORDER BY channel_id, id`, inClause),
		inArgs...,
	)
	if err != nil {
...
func setGroupIDsTx(ctx context.Context, exec dbExec, channelID int64, groupIDs []int64) error {
	...
	for _, groupID := range groupIDs {
		...
		args = append(args, channelID, groupID)
	}
	_, err := exec.ExecContext(ctx,
		"INSERT INTO channel_groups (channel_id, group_id) VALUES "+strings.Join(values, ","),
		args...,
	)
	if err != nil {
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	if err != nil {
		return fmt.Errorf("insert model pricing last id: %w", err)
	}
	pricing.ID = id
	if err := exec.QueryRowContext(ctx, `SELECT created_at, updated_at FROM channel_model_pricing WHERE id = ?`, pricing.ID).Scan(&pricing.CreatedAt, &pricing.UpdatedAt); err != nil {
...
func createIntervalExec(ctx context.Context, exec dbExec, iv *service.PricingInterval) error {
	...
	iv.ID = id
	return exec.QueryRowContext(ctx, `SELECT created_at, updated_at FROM channel_pricing_intervals WHERE id = ?`, iv.ID).Scan(&iv.CreatedAt, &iv.UpdatedAt)
}
