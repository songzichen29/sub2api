...
LEFT JOIN accounts a ON a.id = l.account_id WHERE l.id IS NOT NULL AND (l.request_id LIKE ? OR l.user_email LIKE ? OR l.api_key_name LIKE ? OR l.model LIKE ? OR l.input_excerpt LIKE ?)
ORDER BY l.created_at DESC, l.id DESC
LIMIT ? OFFSET ?`)).
		WithArgs("%abc%", "%abc%", "%abc%", "%abc%", "%abc%", 20, 0).
		WillReturnRows(sqlmock.NewRows([]string{
			"id", "request_id", "user_id", "user_email", "api_key_id", "api_key_name", "account_id", "account_name", "group_id", "group_name",
			...
			int64(1), "req-abc", int64(2), "user@example.com", int64(3), "key-abc", nil, "", int64(4), "default",
...
INSERT INTO content_moderation_logs (
    ...
    ?, ?, ?, ?, ?
		...
		WithArgs(
			"req-1", nil, "u@example.com", nil, "key-a", int64(42), "account-a", nil, "default",
...
func TestContentModerationRepositoryUpdateLogAccount(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	if err != nil {
		t.Fatalf("sqlmock.New: %v", err)
	}
...
WHERE request_id = ? AND api_key_id = ?`)).
		WithArgs(int64(42), "account-a", "client:req-1", int64(3)).
		WillReturnResult(sqlmock.NewResult(0, 1))

	if err := repo.UpdateLogAccount(context.Background(), "client:req-1", 3, 42, "account-a"); err != nil {
		t.Fatalf("UpdateLogAccount error: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("ExpectationsWereMet: %v", err)
	}
