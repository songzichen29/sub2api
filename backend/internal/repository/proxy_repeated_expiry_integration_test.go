...
func (s *ProxyExpirySuite) TestSweep_RepeatedExpiryPreservesOriginalProxy() {
	for _, direct := range []bool{false, true} {
		s.Run(map[bool]string{false: "next backup", true: "direct"}[direct], func() {
			...
			_, err := s.tx.ExecContext(s.ctx, `UPDATE proxies SET backup_proxy_id=? WHERE id=?`, middleID, originalID)
			s.Require().NoError(err)
			if !direct {
				_, err = s.tx.ExecContext(s.ctx, `UPDATE proxies SET fallback_mode='proxy',backup_proxy_id=? WHERE id=?`, lastID, middleID)
				s.Require().NoError(err)
			}
			...
			var origin *int64
			err = scanSingleRow(s.ctx, s.tx, `SELECT proxy_fallback_origin_id FROM accounts WHERE id=?`, []any{accountID}, &origin)
			s.Require().NoError(err)
			s.Require().Equal(&originalID, origin, "manual revert must retain the first original proxy")

			var payloadRaw []byte
			err = scanSingleRow(s.ctx, s.tx, `SELECT payload FROM scheduler_outbox WHERE event_type=? ORDER BY id DESC LIMIT 1`, []any{service.SchedulerOutboxEventAccountBulkChanged}, &payloadRaw)
			s.Require().NoError(err)
			var payload struct {
