...
func TestBulkUpdateOllamaIdentityCleanupIsValueConditional(t *testing.T) {
	...
	require.Contains(t, query, "NOT (COALESCE("+ollamaCloudBaseURLMatchesSQL("credentials ->> 'base_url'")+", false)")
	require.Contains(t, query, "COALESCE("+ollamaCloudBaseURLMatchesSQL("$1::jsonb ->> 'base_url'")+", false)")
	require.NotContains(t, query, "~*")
