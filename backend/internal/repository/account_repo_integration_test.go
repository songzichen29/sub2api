...
func (s *AccountRepoSuite) TestListWithFilters() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, page, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, tt.accType, tt.status, tt.search, tt.groupID, tt.privacyMode, nil)
			s.Require().NoError(err)
...
func (s *AccountRepoSuite) TestListWithFilters_TagsCombined() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, page, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, "", tt.status, "", 0, "", tt.tags)
			s.Require().NoError(err)
			s.Require().Len(accounts, tt.wantCount)
			s.Require().NotNil(page)
			s.Require().Equal(int64(tt.wantCount), page.Total, "total must match items on single page")
