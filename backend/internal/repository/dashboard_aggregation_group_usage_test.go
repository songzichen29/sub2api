...
func TestDashboardAggregationRepositorySyncGroupUsageRollupsNoopsAtCurrentDate(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery(`SELECT CAST\(closed_before AS CHAR\), retained_from.*FOR UPDATE`).
...
func TestDashboardAggregationRepositorySyncGroupUsageRollupsRebuildsWhenTimezoneChanges(t *testing.T) {
	...
	rebuildStart, err := service.ParseGroupUsageDate("2026-03-01")
	require.NoError(t, err)
	for bucketStart := rebuildStart; bucketStart.Before(todayStart); bucketStart = bucketStart.AddDate(0, 0, 1) {
		bucketEnd := bucketStart.AddDate(0, 0, 1)
		mock.ExpectExec(`INSERT INTO usage_group_daily_rollups`).
			WithArgs(service.GroupUsageDate(bucketStart), bucketStart.UTC(), bucketEnd.UTC()).
			WillReturnResult(sqlmock.NewResult(0, 1))
	}
...
func TestDashboardAggregationRepositorySyncGroupUsageRollupsPublishesWatermarkLast(t *testing.T) {
	...
	mock.ExpectExec(`DELETE FROM usage_group_daily_rollups`).
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(`INSERT INTO usage_group_daily_rollups`).
		WithArgs("2026-08-13", rebuildStart, todayStart).
...
func TestDashboardAggregationRepositoryRecomputeRangeInvalidatesGroupRollupsBeforeDashboardRebuild(t *testing.T) {
	...
	mock.ExpectExec(`UPDATE usage_group_rollup_state`).
		WithArgs(service.GroupUsageDate(start)).
		WillReturnResult(sqlmock.NewResult(0, 1))
...
func TestDashboardAggregationRepositoryCleanupUsageLogsNonPartitionedInvalidatesEachBatchAndSyncs(t *testing.T) {
	...
	mock.ExpectExec(`(?s)DELETE FROM usage_logs`).
		...
		WillReturnResult(sqlmock.NewResult(0, 2))
	mock.ExpectExec(`UPDATE usage_group_rollup_state`).
		WithArgs(service.GroupUsageDate(earliestDeletedAt)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
	mock.ExpectBegin()
	mock.ExpectQuery(`SELECT CAST\(closed_before AS CHAR\), retained_from, timezone_name.*FOR UPDATE`).
		WillReturnRows(sqlmock.NewRows([]string{"closed_before", "retained_from", "timezone_name"}).
...
func TestDashboardAggregationRepositoryMySQLUsageLogsAreNotPartitioned(t *testing.T) {
	repo := newDashboardAggregationRepositoryWithSQL(nil)
	partitioned, err := repo.isUsageLogsPartitioned(context.Background())
	require.NoError(t, err)
...
func TestDashboardAggregationRepositoryCleanupUsageLogsNonPartitionedFailureRollsBackWithoutSync(t *testing.T) {
	...
	mock.ExpectExec(`(?s)DELETE FROM usage_logs`).
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(`UPDATE usage_group_rollup_state`).
		WithArgs(service.GroupUsageDate(deletedAt)).
		WillReturnError(sql.ErrConnDone)
	mock.ExpectRollback()
