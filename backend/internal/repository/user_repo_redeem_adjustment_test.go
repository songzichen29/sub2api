...
func TestApplyRedeemBalanceAdjustment_UsesAtomicFloor(t *testing.T) {
	...
	mock.ExpectExec(`UPDATE users SET balance = GREATEST\(balance \+ \?, 0\), updated_at = NOW\(\) WHERE id = \? AND deleted_at IS NULL`).
...
func TestApplyRedeemConcurrencyAdjustment_UsesAtomicFloor(t *testing.T) {
	repo, mock := newRedeemAdjustmentRepoMock(t)
	mock.ExpectExec(`UPDATE users SET concurrency = GREATEST\(concurrency \+ \?, 0\), updated_at = NOW\(\) WHERE id = \? AND deleted_at IS NULL`).
		WithArgs(-7, int64(42)).
		WillReturnResult(sqlmock.NewResult(0, 1))
