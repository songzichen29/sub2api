...
const (
	conditionalBalanceDeductSQL = `(?s)UPDATE users\s+SET balance = balance - \?, updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL AND balance >= \?`
	overdraftBalanceDeductSQL   = `(?s)UPDATE users\s+SET balance = balance - \?, updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL`
	selectBalanceSQL            = `SELECT balance FROM users WHERE id = \?`
	...
	captureBatchImageHoldSQL    = `(?s)UPDATE users\s+SET balance = balance \+ \?,\s+frozen_balance = COALESCE\(frozen_balance, 0\) - \?,\s+updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL AND COALESCE\(frozen_balance, 0\) >= \?`
	...
	selectBalanceFrozenSQL      = `(?s)SELECT balance, COALESCE\(frozen_balance, 0\)\s+FROM users\s+WHERE id = \?`
...
func TestDeductUsageBillingBalance_UsesSufficientBalanceGuard(t *testing.T) {
	...
	mock.ExpectQuery(selectBalanceSQL).
		WithArgs(int64(42)).
...
func TestReserveUsageBillingBatchImageBalance_MovesAvailableToFrozen(t *testing.T) {
	...
	mock.ExpectExec(reserveBatchImageHoldSQL).
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery(selectBalanceFrozenSQL).
		WithArgs(int64(42)).
...
func TestCaptureUsageBillingBatchImageBalance_ReleasesRemainder(t *testing.T) {
	...
	mock.ExpectExec(captureBatchImageHoldSQL).
		WithArgs(0.75, 1.0, int64(42), 1.0).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery(selectBalanceFrozenSQL).
		WithArgs(int64(42)).
