...
func (s *UserSubscriptionRepoSuite) TestIncrementUsage_DisabledDayOverdraftRepaysDebt() {
	user := s.mustCreateUser("disabled-overdraft-debt@test.com", service.RoleUser)
	group := s.mustCreateGroup("g-disabled-overdraft-debt")
	daily := 80.0
	...
	sub := s.mustCreateSubscription(user.ID, group.ID, func(c *dbent.UserSubscriptionCreate) {
		c.SetStartsAt(startsAt).
			...
			SetDailyUsageUsd(0).
			SetWeeklyUsageUsd(330).
			SetValidityUnit("day").
			SetAllowDailyOverdraft(false)
	})

	s.Require().NoError(s.repo.IncrementUsage(s.ctx, sub.ID, 70))
...
func (s *UserSubscriptionRepoSuite) TestIncrementUsage_OneTimeDailyQuotaMarksExhausted() {
	user := s.mustCreateUser("one-time-daily-quota@test.com", service.RoleUser)
	group := s.mustCreateGroup("g-one-time-daily-quota")
	daily := 80.0
	_, err := s.client.Group.UpdateOneID(group.ID).
		SetDailyLimitUsd(daily).
		Save(s.ctx)
	...
	sub := s.mustCreateSubscription(user.ID, group.ID, func(c *dbent.UserSubscriptionCreate) {
		c.SetStartsAt(startsAt).
			SetExpiresAt(startsAt.Add(24 * time.Hour)).
			SetDailyWindowStart(startsAt).
			SetDailyUsageUsd(79).
			SetWeeklyUsageUsd(79).
			SetValidityUnit("day")
	})

	s.Require().NoError(s.repo.IncrementUsage(s.ctx, sub.ID, 1))
	got, err := s.repo.GetByID(s.ctx, sub.ID)
	s.Require().NoError(err)
	s.Require().Equal(service.SubscriptionStatusQuotaExhausted, got.Status)
}
...
func (s *UserSubscriptionRepoSuite) TestActivateWindows_StaleActivationPreservesExistingWindows() {
	...
	s.Require().NoError(s.repo.ActivateWindows(s.ctx, sub.ID, activatedAt, activatedAt, activatedAt))
	s.Require().NoError(s.repo.ResetUsageWindows(s.ctx, sub.ID, true, true, true, manualDailyStart, manualResetAt, manualResetAt))
	// Simulate a concurrent request carrying the original unactivated snapshot.
	s.Require().NoError(s.repo.ActivateWindows(s.ctx, sub.ID, activatedAt.Add(time.Hour), activatedAt.Add(time.Hour), activatedAt.Add(time.Hour)))
...
func (s *UserSubscriptionRepoSuite) TestResetUsageWindows_ClearsUsageAfterAutomaticWindowAdvance() {
	...
	s.Require().NoError(s.repo.ResetDailyUsage(s.ctx, sub.ID, &oldWindowStart, newWindowStart))
	s.Require().NoError(s.repo.IncrementUsage(s.ctx, sub.ID, 3))
	s.Require().NoError(s.repo.ResetUsageWindows(s.ctx, sub.ID, true, false, false, newWindowStart, newWindowStart, newWindowStart))
...
func (s *UserSubscriptionRepoSuite) TestActiveExpiredBoundaries_UsageAndReset_BatchUpdateExpiredStatus() {
	...
	s.Require().NoError(s.repo.ActivateWindows(s.ctx, active.ID, activateAt, activateAt, activateAt), "ActivateWindows")
