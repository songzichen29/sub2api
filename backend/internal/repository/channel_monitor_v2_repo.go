...
func (r *channelMonitorV2Repository) GetConfig(ctx context.Context) (*service.ChannelMonitorV2Config, error) {
	return scanChannelMonitorV2Config(r.db.QueryRowContext(ctx, `
		SELECT version, enabled, refresh_interval_seconds,
		       COALESCE(platforms, JSON_ARRAY()), COALESCE(group_ids, JSON_ARRAY()),
		       COALESCE(ignored_error_categories, JSON_ARRAY()),
		       COALESCE(health_thresholds, JSON_OBJECT()),
		       updated_at, updated_by
		FROM channel_monitor_v2_config WHERE id = 1`))
}

type channelMonitorV2RowScanner interface {
	Scan(dest ...any) error
}

func scanChannelMonitorV2Config(row channelMonitorV2RowScanner) (*service.ChannelMonitorV2Config, error) {
	var cfg service.ChannelMonitorV2Config
	var platforms, groupIDs, ignoredCategories, thresholds []byte
	err := row.Scan(
		&cfg.Version, &cfg.Enabled, &cfg.RefreshIntervalSeconds,
		&platforms, &groupIDs, &ignoredCategories, &thresholds,
	...
	if err := json.Unmarshal(groupIDs, &cfg.GroupIDs); err != nil {
		return nil, fmt.Errorf("decode channel monitor v2 group ids: %w", err)
	}
	if err := json.Unmarshal(ignoredCategories, &cfg.IgnoredErrorCategories); err != nil {
...
func (r *channelMonitorV2Repository) UpdateConfig(ctx context.Context, cfg service.ChannelMonitorV2Config, expectedVersion int) (*service.ChannelMonitorV2Config, error) {
	...
	result, err := tx.ExecContext(ctx, `
		...
		SET version = version + 1, enabled = ?, refresh_interval_seconds = ?,
		    ...
		    health_thresholds = ?, updated_by = ?, updated_at = NOW(6)
	...
	updated, err := scanChannelMonitorV2Config(tx.QueryRowContext(ctx, `
		SELECT version, enabled, refresh_interval_seconds,
		       COALESCE(platforms, JSON_ARRAY()), COALESCE(group_ids, JSON_ARRAY()),
		       COALESCE(ignored_error_categories, JSON_ARRAY()),
		       COALESCE(health_thresholds, JSON_OBJECT()),
...
func (r *channelMonitorV2Repository) loadErrorDetails(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config) (map[string][]service.ChannelMonitorV2ErrorDetail, error) {
	...
	if len(platforms) > 0 {
		...
		conditions = append(conditions, "lower(COALESCE(NULLIF(TRIM(current_error.platform), ''), 'unknown')) IN ("+sqlPlaceholders(len(platforms))+")")
	} else {
	...
	if groupScopeEmpty {
		...
		conditions = append(conditions, "COALESCE(current_error.group_id, 0) IN ("+sqlPlaceholders(len(groups))+")")
	}
	query := `SELECT
...
func (r *channelMonitorV2Repository) loadFacts(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, byBucket bool) ([]channelMonitorV2Fact, error) {
	...
	if byBucket {
		if bucketSeconds > 0 {
			...
			bucketExpr = channelMonitorV2BucketExpr("m.bucket_start", int(filter.Bucket.Seconds()))
			group = bucketExpr + "," + group
		}
	}
...
func (r *channelMonitorV2Repository) loadHistograms(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, userID int64, byBucket bool) ([]channelMonitorV2Histogram, error) {
	...
	if byBucket {
		if bucketSeconds > 0 {
			...
			bucketExpr = channelMonitorV2BucketExpr("h.bucket_start", int(filter.Bucket.Seconds()))
			group = bucketExpr + "," + group
		}
	}
...
func channelMonitorV2BucketExpr(column string, seconds int) string {
	if seconds <= 0 {
		return column
	}
	return fmt.Sprintf(
		"TIMESTAMPADD(SECOND, FLOOR(TIMESTAMPDIFF(SECOND, '1970-01-01 00:00:00', %s) / %d) * %d, '1970-01-01 00:00:00')",
...
// fixed epoch alignment as loadFacts when filter.Bucket is set.
func (r *channelMonitorV2Repository) loadIgnoredErrorCounts(
	...
	if filter.Bucket > 0 && bucketSeconds == 0 {
		bucketExpr = channelMonitorV2BucketExpr("e.bucket_start", int(filter.Bucket.Seconds()))
	...
	query := fmt.Sprintf(
		`SELECT %s, e.platform, e.model, SUM(e.error_requests)
		 ...
		 GROUP BY %s`,
		bucketExpr, where, sqlPlaceholders(len(cfg.IgnoredErrorCategories)), groupBy,
	)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
...
func (r *channelMonitorV2Repository) loadIgnoredErrorCountsByPlatformModel(
	...
	query := fmt.Sprintf(
		...
		where, sqlPlaceholders(len(cfg.IgnoredErrorCategories)),
	)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
		return byPM, 0, err
...
// and by (dimension, bucket). Uses the same fixed epoch alignment as GetMatrix facts.
func (r *channelMonitorV2Repository) loadIgnoredErrorCountsByMatrixKey(
	...
	query := fmt.Sprintf(
		`SELECT %s, e.platform, e.group_id, e.model, SUM(e.error_requests)
		 ...
		 AND e.error_category IN (%s) AND e.taxonomy_version = ?
		 GROUP BY %s`,
		bucketExpr, where, sqlPlaceholders(len(cfg.IgnoredErrorCategories)), groupSQL,
	)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
