...
func (r *channelRepository) batchLoadAccountStatsPricingRules(ctx context.Context, channelIDs []int64) (map[int64][]service.AccountStatsPricingRule, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		fmt.Sprintf(`SELECT id, channel_id, name, group_ids, account_ids, sort_order, created_at, updated_at FROM channel_account_stats_pricing_rules WHERE channel_id IN (%s) ORDER BY channel_id, sort_order, id`, inClause),
		inArgs...,
	)
	if err != nil {
...
func (r *channelRepository) batchLoadAccountStatsModelPricing(ctx context.Context, ruleIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	...
	inClause, inArgs := buildChannelInt64InClause(ruleIDs)
	rows, err := r.db.QueryContext(ctx,
		fmt.Sprintf(`SELECT id, rule_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_output_price, per_request_price, created_at, updated_at FROM channel_account_stats_model_pricing WHERE rule_id IN (%s) ORDER BY rule_id, id`, inClause),
		inArgs...,
	)
	if err != nil {
...
func createAccountStatsPricingRuleTx(ctx context.Context, tx *sql.Tx, rule *service.AccountStatsPricingRule) error {
	...
	res, err := tx.ExecContext(ctx,
		`INSERT INTO channel_account_stats_pricing_rules (channel_id, name, group_ids, account_ids, sort_order) VALUES (?, ?, ?, ?, ?)`,
	...
	if err != nil {
		return fmt.Errorf("account stats pricing rule last id: %w", err)
	}
	rule.ID = id
	if err := tx.QueryRowContext(ctx, `SELECT created_at, updated_at FROM channel_account_stats_pricing_rules WHERE id = ?`, rule.ID).Scan(&rule.CreatedAt, &rule.UpdatedAt); err != nil {
		return err
	}
...
func createAccountStatsModelPricingTx(ctx context.Context, tx *sql.Tx, ruleID int64, pricing *service.ChannelModelPricing) error {
	...
	pricing.ID = id
	if err := tx.QueryRowContext(ctx, `SELECT created_at, updated_at FROM channel_account_stats_model_pricing WHERE id = ?`, pricing.ID).Scan(&pricing.CreatedAt, &pricing.UpdatedAt); err != nil {
		return err
	}
...
func createAccountStatsIntervalTx(ctx context.Context, tx *sql.Tx, iv *service.PricingInterval) error {
	...
	iv.ID = id
	return tx.QueryRowContext(ctx, `SELECT created_at, updated_at FROM channel_account_stats_pricing_intervals WHERE id = ?`, iv.ID).Scan(&iv.CreatedAt, &iv.UpdatedAt)
}
...
func (r *channelRepository) batchLoadAccountStatsIntervals(ctx context.Context, pricingIDs []int64) (map[int64][]service.PricingInterval, error) {
	...
	inClause, inArgs := buildChannelInt64InClause(pricingIDs)
	rows, err := r.db.QueryContext(ctx,
		fmt.Sprintf(`SELECT id, pricing_id, min_tokens, max_tokens, tier_label, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, per_request_price, sort_order, created_at, updated_at FROM channel_account_stats_pricing_intervals WHERE pricing_id IN (%s) ORDER BY pricing_id, sort_order, id`, inClause),
		inArgs...,
	)
	if err != nil {
