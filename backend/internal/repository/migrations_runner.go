...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	// 076 originally used unsupported MySQL ADD COLUMN IF NOT EXISTS syntax;
	// keep databases that recorded the original migration checksum compatible.
	"076_channel_monitor_v2.sql":                              newMigrationChecksumCompatibilityRule("988d13f8dfea063b4246f6b24027f24de329b819cd90ece29be33deb01515678", "8b9b37935c1e378ca4a8f7461487d391c2995bcec0898a70e5601932c0e5e93e"),
	"079_user_platform_quotas_cn_providers.sql":               newMigrationChecksumCompatibilityRule("d5be459ea6a5b0089ede375724a0361ad5252a93ede0354f00c718a079c62e04", "a5092b9cfdc0694ee31af580143fa7aa341e1638370d2a577a5847c4ae28be2c"),
...
func applyMigrationsFS(ctx context.Context, db *sql.DB, fsys fs.FS) error {
	...
	defer func() {
		...
		defer cancel()
		_ = releaseMigrationsLock(releaseCtx, lockConn)
	}()
	...
	for _, name := range files {
		...
		// 检查该迁移是否已经应用
		var existing string
		rowErr := lockConn.QueryRowContext(ctx, "SELECT checksum FROM schema_migrations WHERE filename = ?", name).Scan(&existing)
		if rowErr == nil {
			// 迁移已应用，验证校验和是否匹配
		...
		if nonTx {
			...
			for i, stmt := range statements {
				...
				if _, err := lockConn.ExecContext(ctx, executable); err != nil {
					return fmt.Errorf("apply migration %s (non-tx statement %d): %w", name, i+1, err)
				}
			}
			if _, err := lockConn.ExecContext(ctx, "INSERT INTO schema_migrations (filename, checksum) VALUES (?, ?)", name, checksum); err != nil {
...
type migrationsLockConnection interface {
	QueryRowContext(ctx context.Context, query string, args ...any) *sql.Row
}

// acquireMigrationsLock 获取 MySQL 命名锁。
func acquireMigrationsLock(ctx context.Context, db migrationsLockConnection) error {
...
func releaseMigrationsLock(ctx context.Context, db migrationsLockConnection) error {
