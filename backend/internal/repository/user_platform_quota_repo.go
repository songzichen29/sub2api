...
type UserPlatformQuotaRepository interface {
	// BulkInsertInitial 幂等批量插入初始配额记录（ON DUPLICATE KEY DO NOTHING）。
...
// BulkInsertInitial 用原生 SQL ON DUPLICATE KEY 实现幂等批量插入（带条件 limit 覆盖）。
...
func (r *userPlatformQuotaRepository) IncrementUsageWithReset(ctx context.Context, userID int64, platform string, cost float64, now time.Time) error {
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		if dbent.IsNotFound(err) {
			...
			// 用 ON DUPLICATE KEY DO UPDATE 累加，而非裸 INSERT：并发下另一请求可能在本事务
			// SELECT FOR UPDATE 之后、INSERT 之前刚建行，裸 INSERT 会撞 partial unique index
			// 致事务回滚、本次 cost 丢失；DO UPDATE 把 cost 累加到既有 usage 上。
			// 写法与本文件 insertLimitsRow / BulkInsertInitial 的 ON DUPLICATE KEY 一致。
...
// 保证事务内所有时间戳一致（避免 MySQL NOW() 与 Go time.Now() 的微小偏差）。
...
// 带 ON DUPLICATE KEY ... DO NOTHING 守卫：防止两个并发请求同时为同一 user/platform 新建行时
...
// batchRows 是 BatchSnapshotUsage 每批最大行数（9 参/行 × 6000 ≈ 54000 参,低于 MySQL 65535 上限）。
