...
const (
	// Keep the PostgreSQL expressions as the default names because SQL-only
	// repository tests (and callers without an Ent client) use PostgreSQL.
...
func ollamaCloudBaseURLMatchesMySQL(expression string) string {
	return ollamaCloudBaseURLMatchMySQLPrefix + expression + ollamaCloudBaseURLMatchMySQLSuffix
}

func accountRepositoryDialect(r *accountRepository) string {
...
func sameOllamaCloudUsageGroupIdentity(left, right *service.Account) bool {
	...
	leftKey, leftOK := left.Credentials["api_key"].(string)
	rightKey, rightOK := right.Credentials["api_key"].(string)
	return leftOK && rightOK && leftKey != "" && leftKey == rightKey
}

func invalidateOllamaCloudUsageForBulkUpdateMySQL(ctx context.Context, exec sqlExecutor, ids []int64, updates service.AccountBulkUpdate) error {
	if exec == nil || len(ids) == 0 {
		return nil
	}
	placeholders, idArgs := buildAccountInt64InClause(ids)
	groupIdentityChanged := ""
	groupArgs := make([]any, 0, 1+len(idArgs))
	if _, changesAPIKey := updates.Credentials["api_key"]; changesAPIKey {
		payload, err := json.Marshal(updates.Credentials)
		if err != nil {
			return err
		}
		groupIdentityChanged = "NOT (JSON_EXTRACT(credentials, '$.api_key') <=> JSON_EXTRACT(CAST(? AS JSON), '$.api_key'))"
		groupArgs = append(groupArgs, string(payload))
	}
	if rawBaseURL, changesBaseURL := updates.Credentials["base_url"]; changesBaseURL {
		baseURL, _ := rawBaseURL.(string)
		candidate := &service.Account{
			Platform: service.PlatformOpenAI,
			Type:     service.AccountTypeAPIKey,
			Credentials: map[string]any{
				"api_key":  "identity-check",
				"base_url": baseURL,
			},
		}
		if !service.IsOllamaCloudUsageAccount(candidate) {
			groupIdentityChanged = "TRUE"
			groupArgs = groupArgs[:0]
		}
	}
	if groupIdentityChanged != "" {
		groupArgs = append(groupArgs, idArgs...)
		if _, err := exec.ExecContext(ctx, `
			...
			WHERE deleted_at IS NULL
				AND `+ollamaCloudUsageEligibleMySQL+`
				AND `+groupIdentityChanged+`
	...
	proxyChanged := "proxy_id IS NOT NULL"
	proxyArgs := make([]any, 0, 1+len(idArgs))
	if *updates.ProxyID != 0 {
		proxyChanged = "NOT (proxy_id <=> ?)"
		proxyArgs = append(proxyArgs, *updates.ProxyID)
	}
	proxyArgs = append(proxyArgs, idArgs...)
	_, err := exec.ExecContext(ctx, `
		UPDATE accounts
		SET extra = JSON_REMOVE(COALESCE(extra, JSON_OBJECT()), '$.ollama_cloud_usage_snapshot'),
			updated_at = NOW(6)
		WHERE deleted_at IS NULL
			AND `+ollamaCloudUsageEligibleMySQL+`
			AND `+proxyChanged+`
...
func (r *accountRepository) ListOllamaCloudUsageGroupAccounts(ctx context.Context, accounts []*service.Account) ([]service.Account, error) {
	if accountRepositoryDialect(r) == dialect.MySQL {
		return r.listOllamaCloudUsageGroupAccountsMySQL(ctx, accounts)
	}
	return r.listOllamaCloudUsageGroupAccountsPostgres(ctx, accounts)
}

func (r *accountRepository) listOllamaCloudUsageGroupAccountsPostgres(ctx context.Context, accounts []*service.Account) ([]service.Account, error) {
	if r == nil || r.sql == nil {
...
func (r *accountRepository) listOllamaCloudUsageGroupAccountsMySQL(ctx context.Context, accounts []*service.Account) ([]service.Account, error) {
	if r == nil || r.sql == nil {
		return nil, service.ErrOllamaCloudUsageUnavailable
	}
	keys := make([]string, 0, len(accounts))
	...
	rows, err := r.sql.QueryContext(ctx, `
		...
		WHERE deleted_at IS NULL
			AND `+ollamaCloudUsageEligibleMySQL+`
			AND JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.api_key')) IN (`+placeholders+`)
		ORDER BY id
	`, args...)
	if err != nil {
		return nil, err
...
func (r *accountRepository) updateOllamaCloudUsageGroup(
	...
	apply := func(txCtx context.Context, client *dbent.Client) error {
		...
		if dbDialect == dialect.MySQL {
			placeholders, idArgs := buildAccountInt64InClause(memberIDs)
			args := make([]any, 0, 2+len(idArgs))
			args = append(args, string(encoded), apiKey)
...
func lockOllamaCloudUsageGroup(
	...
	if client != nil && client.Driver() != nil && client.Driver().Dialect() == dialect.MySQL {
		return lockOllamaCloudUsageGroupMySQL(ctx, client, account, apiKey)
	}
	return lockOllamaCloudUsageGroupPostgres(ctx, client, account, apiKey)
}

func lockOllamaCloudUsageGroupPostgres(
	ctx context.Context,
	client *dbent.Client,
	account *service.Account,
	apiKey string,
...
func lockOllamaCloudUsageGroupMySQL(
	ctx context.Context,
	client *dbent.Client,
	account *service.Account,
	apiKey string,
	...
	rows, err := client.QueryContext(ctx, `
		SELECT
			...
			id = ?
				...
				AND proxy_id <=> ?,
			COALESCE(JSON_EXTRACT(extra, '$.ollama_cloud_usage_session'), CAST('null' AS JSON)),
			COALESCE(JSON_EXTRACT(extra, '$.ollama_cloud_usage_auto_refresh'), CAST('null' AS JSON)),
			COALESCE(JSON_EXTRACT(extra, '$.ollama_cloud_usage_snapshot'), CAST('null' AS JSON))
		FROM accounts
		WHERE deleted_at IS NULL
			AND `+ollamaCloudUsageEligibleMySQL+`
			AND JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.api_key')) = ?
...
func (r *accountRepository) ListDueOllamaCloudUsageAccounts(
	...
	if accountRepositoryDialect(r) == dialect.MySQL {
		return r.listDueOllamaCloudUsageAccountsMySQL(ctx, now, debounce, maxWait, limit)
	}
	return r.listDueOllamaCloudUsageAccountsPostgres(ctx, now, debounce, maxWait, limit)
}

func (r *accountRepository) listDueOllamaCloudUsageAccountsMySQL(
	ctx context.Context,
	now time.Time,
	debounce, maxWait time.Duration,
	limit int,
	...
	aEligible := ollamaCloudUsageEligibleMySQLForAlias("a")
	gEligible := ollamaCloudUsageEligibleMySQLForAlias("g")
	rows, err := r.sql.QueryContext(ctx, `
		SELECT
			a.id,
			JSON_UNQUOTE(JSON_EXTRACT(a.credentials, '$.api_key')) AS api_key,
			MAX(g.last_used_at) AS group_last_used_at,
			JSON_EXTRACT(a.extra, '$.ollama_cloud_usage_snapshot') AS snapshot
		FROM accounts a
		JOIN accounts g
			ON g.deleted_at IS NULL
			AND `+gEligible+`
			AND JSON_UNQUOTE(JSON_EXTRACT(g.credentials, '$.api_key')) =
				JSON_UNQUOTE(JSON_EXTRACT(a.credentials, '$.api_key'))
		WHERE a.deleted_at IS NULL
			AND a.status = 'active'
			AND `+aEligible+`
			AND JSON_TYPE(JSON_EXTRACT(a.extra, '$.ollama_cloud_usage_session')) = 'STRING'
			AND JSON_UNQUOTE(JSON_EXTRACT(a.extra, '$.ollama_cloud_usage_auto_refresh')) = 'true'
		GROUP BY a.id,
			JSON_UNQUOTE(JSON_EXTRACT(a.credentials, '$.api_key')),
	...
	selected := make([]dueRow, 0, limit)
	selectedKeys := make(map[string]struct{}, limit)
	for rows.Next() {
		var id int64
		var apiKey string
		var groupLastUsed sql.NullTime
		var snapshotRaw []byte
		if err := rows.Scan(&id, &apiKey, &groupLastUsed, &snapshotRaw); err != nil {
			return nil, err
		}
		if _, exists := selectedKeys[apiKey]; exists {
			continue
		}
		var snapshot *service.OllamaCloudUsageSnapshot
		if len(snapshotRaw) > 0 && string(snapshotRaw) != "null" {
			var decoded service.OllamaCloudUsageSnapshot
			if err := json.Unmarshal(snapshotRaw, &decoded); err == nil {
				snapshot = &decoded
			}
		}
		var lastUsed *time.Time
		if groupLastUsed.Valid {
			ts := groupLastUsed.Time.UTC()
			lastUsed = &ts
		}
		if !service.OllamaCloudUsageIsAutoRefreshDue(snapshot, lastUsed, now.UTC(), debounce, maxWait) {
			continue
		}
		selectedKeys[apiKey] = struct{}{}
		selected = append(selected, dueRow{id: id, apiKey: apiKey, groupLastUsed: lastUsed})
	...
	result := make([]service.Account, 0, len(selected))
	for _, row := range selected {
		account := byID[row.id]
		if account == nil {
			continue
		}
		account.LastUsedAt = row.groupLastUsed
		result = append(result, *account)
	}
	return result, nil
}

func ollamaCloudUsageEligibleMySQLForAlias(alias string) string {
...
func (r *accountRepository) listDueOllamaCloudUsageAccountsPostgres(
	ctx context.Context,
	now time.Time,
	debounce, maxWait time.Duration,
	limit int,
