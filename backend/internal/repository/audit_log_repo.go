...
func buildAuditLogsWhere(filter *service.AuditLogFilter) (string, []any) {
	...
	if filter.ActorUserID != nil {
		...
		clauses = append(clauses, "l.actor_user_id = ?")
	}
	if v := strings.TrimSpace(filter.ActorEmail); v != "" {
		args = append(args, "%"+escapeLikePattern(v)+"%")
		clauses = append(clauses, "LOWER(l.actor_email) LIKE LOWER(?)")
	}
	if v := strings.TrimSpace(filter.AuthMethod); v != "" {
		args = append(args, v)
		clauses = append(clauses, "l.auth_method = ?")
	}
	if v := strings.TrimSpace(filter.Action); v != "" {
		args = append(args, "%"+escapeLikePattern(v)+"%")
		clauses = append(clauses, "LOWER(l.action) LIKE LOWER(?)")
	}
	...
	if v := strings.TrimSpace(filter.ClientIP); v != "" {
		args = append(args, v)
		clauses = append(clauses, "l.client_ip = ?")
	}
	...
	if v := strings.TrimSpace(filter.Query); v != "" {
		args = append(args, "%"+escapeLikePattern(v)+"%")
		clauses = append(clauses, "(LOWER(l.path) LIKE LOWER(?) OR LOWER(l.action) LIKE LOWER(?) OR LOWER(l.actor_email) LIKE LOWER(?))")
...
func (r *auditLogRepository) DeleteBefore(ctx context.Context, cutoff time.Time, batchSize int) (int64, error) {
	...
	res, err := r.db.ExecContext(ctx,
		`DELETE FROM audit_logs WHERE created_at < ? ORDER BY id LIMIT ?`, cutoff.UTC(), batchSize)
	if err != nil {
		return 0, err
	}
