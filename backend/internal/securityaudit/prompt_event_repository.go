...
func (r *PostgreSQLRepository) ListEvents(ctx context.Context, filter EventFilter, page, pageSize int) (*EventPage, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT `+eventColumns("e")+` FROM prompt_audit_events e`+where+
		` ORDER BY e.created_at DESC, e.id DESC LIMIT ? OFFSET ?`, queryArgs...)
	if err != nil {
		return nil, err
	}
...
func (r *PostgreSQLRepository) DeleteEventsByIDs(ctx context.Context, ids []int64) (*DeleteResult, error) {
	...
	args := int64Args(ids)
	rows, err := tx.QueryContext(ctx, `SELECT job_id FROM prompt_audit_events WHERE id IN (`+placeholders+`) FOR UPDATE`, args...)
	if err != nil {
		return nil, err
	}
	...
	deleteResult, err := tx.ExecContext(ctx, `DELETE FROM prompt_audit_events WHERE id IN (`+placeholders+`)`, args...)
	if err != nil {
		return nil, err
	}
	deletedEvents, err := deleteResult.RowsAffected()
	if err != nil {
		return nil, err
	}
	...
	return &DeleteResult{DeletedEvents: deletedEvents, DeletedJobs: deletedJobs, JobIDs: canonicalInt64s(jobIDs)}, nil
}
...
func (r *PostgreSQLRepository) DeleteEventsByFilter(ctx context.Context, filter EventFilter, snapshotMaxID int64, batchSize int) (*DeleteResult, error) {
	...
	for {
		...
		rows, err := tx.QueryContext(ctx, `
			...
			AND e.id <= ? ORDER BY e.id LIMIT ? FOR UPDATE SKIP LOCKED`, args...)
		if err != nil {
			_ = tx.Rollback()
			return nil, err
		}
		eventIDs, jobIDs, err := scanPromptAuditDeleteRows(rows)
		if err != nil {
			_ = tx.Rollback()
			return nil, err
		}
		if len(eventIDs) > 0 {
			deleteArgs := int64Args(eventIDs)
			if _, err := tx.ExecContext(ctx, `DELETE FROM prompt_audit_events WHERE id IN (`+sqlPlaceholders(len(eventIDs))+`)`, deleteArgs...); err != nil {
				_ = tx.Rollback()
				return nil, err
			}
		}
...
func buildEventWhere(filter EventFilter, _ int) (string, []any) {
	...
	if filter.Decision != "" {
		add(" AND e.decision=?", filter.Decision)
	}
	if filter.RiskLevel != "" {
		add(" AND e.risk_level=?", filter.RiskLevel)
	}
	if filter.Endpoint != "" {
		add(" AND e.endpoint=?", filter.Endpoint)
	}
	if filter.GroupID != nil {
		add(" AND e.group_id=?", *filter.GroupID)
	}
	if filter.UserID != nil {
		add(" AND e.user_id=?", *filter.UserID)
	}
	if filter.APIKeyID != nil {
		add(" AND e.api_key_id=?", *filter.APIKeyID)
	}
	if filter.RequestID != "" {
		add(" AND e.request_id=?", filter.RequestID)
	}
	if filter.PromptHash != "" {
		add(" AND e.prompt_hash=?", filter.PromptHash)
	}
	if filter.Keyword != "" {
		value := "%" + TrimRunes(filter.Keyword, 128) + "%"
		clauses = append(clauses, ` AND (LOWER(e.request_id) LIKE LOWER(?) OR LOWER(e.prompt_hash) LIKE LOWER(?) OR LOWER(e.redacted_preview) LIKE LOWER(?)
			OR LOWER(e.username_snapshot) LIKE LOWER(?) OR LOWER(e.user_email_snapshot) LIKE LOWER(?) OR LOWER(e.api_key_name_snapshot) LIKE LOWER(?))`)
		for range 6 {
			args = append(args, value)
		}
	}
	if filter.StartAt != nil {
		add(" AND e.created_at >= ?", filter.StartAt.UTC())
	}
	if filter.EndAt != nil {
		add(" AND e.created_at <= ?", filter.EndAt.UTC())
	}
...
func scanPromptAuditDeleteRows(rows *sql.Rows) ([]int64, []int64, error) {
	defer func() { _ = rows.Close() }()
	...
	return eventIDs, jobIDs, rows.Err()
}
...
func deleteOrphanJobs(ctx context.Context, tx *sql.Tx, jobIDs []int64) (int64, error) {
	...
	args := int64Args(jobIDs)
	result, err := tx.ExecContext(ctx, `DELETE j FROM prompt_audit_jobs j
		WHERE j.id IN (`+sqlPlaceholders(len(jobIDs))+`) AND j.status <> 'processing'
		AND NOT EXISTS (SELECT 1 FROM prompt_audit_events e WHERE e.job_id=j.id)`, args...)
	if err != nil {
		return 0, err
	}
