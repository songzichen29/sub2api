...
func (m *ConfigManager) Save(ctx context.Context, req UpdateConfigRequest, actorID int64) (PublicConfig, error) {
	...
	conn, err := acquirePromptAuditNamedLock(ctx, m.db, promptAuditConfigLockName)
	if err != nil {
		return PublicConfig{}, err
	}
	defer releasePromptAuditNamedLock(conn, promptAuditConfigLockName)
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO settings (`+"`key`"+`,value,updated_at) VALUES (?,?,NOW(6))
