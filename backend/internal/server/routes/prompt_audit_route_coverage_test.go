...
func TestResponsesWebSocketHasFirstAndSubsequentTurnPromptGates(t *testing.T) {
	...
	userSlotIndex := strings.Index(wsSource, `TryAcquireUserSlotForAPIKey`)
	if userSlotIndex == -1 {
		userSlotIndex = strings.Index(wsSource, `TryAcquireUserSlot(`)
	}
	require.NotEqual(t, -1, userSlotIndex, "the WebSocket handler must acquire a per-request user slot")
	require.Less(t,
