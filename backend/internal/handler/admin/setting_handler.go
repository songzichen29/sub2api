...
// SetAliyunCaptchaService attaches the Aliyun captcha credential validator.
...
func (h *SettingHandler) UpdateAccountImportTemplates(c *gin.Context) {
	...
	templates, err := h.settingService.UpdateAccountImportApplyTemplates(c.Request.Context(), req.Templates)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
