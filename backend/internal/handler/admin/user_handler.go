...
func NewUserHandler(
	...
	if len(optional) > 0 {
		if repo, ok := optional[0].(service.UserPlatformQuotaRepository); ok {
			userPlatformQuotaRepo = repo
		}
	}
	if len(optional) > 1 {
		if cache, ok := optional[1].(service.BillingCache); ok {
			billingCache = cache
		}
	}
	if len(optional) > 2 {
		totpService, _ = optional[2].(*service.TotpService)
	}
	if len(optional) > 3 {
		userService, _ = optional[3].(*service.UserService)
	}
	if len(optional) > 4 {
		settingService, _ = optional[4].(*service.SettingService)
	}
	return &UserHandler{
