...
func setupAdminRouter() (*gin.Engine, *stubAdminService) {
	...
	router.GET("/api/v1/admin/groups/:id/models-list-candidates", groupHandler.GetGroupModelAllowlistCandidates)
...
func TestGroupHandlerEndpoints(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodGet, "/api/v1/admin/groups/0/models-list-candidates?platform=anthropic", nil)
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
