...
// AccountRuntimeBlocker optionally mirrors durable account scheduling state into
// an in-memory fast path used by OpenAI account scheduling.
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	if len(requestedModel) > 0 && strings.TrimSpace(requestedModel[0]) != "" {
		if s.HandleUpstreamModelNotFound(ctx, account, requestedModel[0], statusCode, responseBody) {
			return true
		}
	}
	...
	if statusCode != 401 && !isOpenAIAPIKeyQuotaExhausted(account, "", responseBody) {
	...
	case 401:
		...
		if authAccount.Type == AccountTypeOAuth && authAccount.Platform != PlatformAntigravity {
			...
			// 注意：此处不再写回 account.Credentials/expires_at，避免用请求开始时的
			// credentials 快照整列覆盖另一个 worker 刚刷新出的新 refresh_token。
...
func isOpenAIAPIKeyQuotaExhausted(account *Account, upstreamMsg string, responseBody []byte) bool {
	...
	for _, needle := range quotaExhaustedSignals {
...
// headers (Fable-only 7d window). Exhaustion of this window only limits the
// Fable model family; the account remains schedulable for other models.
...
// when the 7d_oi window is exhausted. Returns true when 7d_oi was present so
// callers do not fall through into generic 429 handling that would limit the
// whole account or trigger local temp-unsched rules.
