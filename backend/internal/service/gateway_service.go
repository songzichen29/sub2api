...
var (
	...
	// Deprecated: flusher_enabled=true 后不再增长（仅 flag=false 降级直写路径使用）；新主路径见 FlusherMetrics。
...
func GatewayUserPlatformQuotaFlusherStats(f *UserPlatformQuotaUsageFlusher) map[string]int64 {
	...
	return map[string]int64{
		...
		"fk_violation":         m.FlushFKViolationTotal.Load(),
	}
}
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if account != nil && parsed.Body != nil && s.shouldEmulateWebSearch(ctx, account, parsed.GroupID, parsed.Body.Bytes()) {
	...
	if account != nil && s.isBedrockCCCompatEnabled(ctx, account, parsed.GroupID) {
		...
		body = sanitizeBedrockThinking(body, parsed.Model)
		body = sanitizeBedrockToolUseIDs(body)
		_ = parsed.ReplaceBody(body)
	}
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	// CCH: sign only when explicitly enabled; otherwise remove the placeholder.
	if enableCCH {
	...
	// Compute final anthropic-beta before request creation so body fields incompatible
	// with dropped beta tokens can be sanitized consistently.
	...
	// OAuth + mimic Claude Code: apply captured CLI-like headers.
	if tokenType == "oauth" && mimicClaudeCode {
		applyClaudeCodeMimicHeaders(req, false)
	}

	// Apply final anthropic-beta header.
...
// computeFinalAnthropicBeta computes the final anthropic-beta header for /v1/messages.
...
// computeFinalCountTokensAnthropicBeta computes the final anthropic-beta header for count_tokens.
...
type RecordUsageInput struct {
	...
	QuotaPlatform      string             // user×platform 配额计量平台；handler 需在请求 ctx 内算定后传入
...
type postUsageBillingParams struct {
	...
	Platform              string // 来自 APIKey 关联 Group 或 ForcePlatform 的平台标识
}
...
// 强制平台路由（如 /antigravity）优先按 ctx 中的 ForcePlatform 计量，否则回退到 APIKey 关联 Group 的平台。
...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	if !p.IsSubscriptionBill && p.Platform != "" && cost.ActualCost > 0 && p.User != nil && deps.userPlatformQuotaRepo != nil && deps.billingCacheService != nil {
...
func markSubscriptionQuotaExhaustedAfterBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	if p.Subscription.HasTotalQuotaLimit() && p.Subscription.QuotaUsedUSD+p.Cost.ActualCost >= *p.Subscription.QuotaLimitUSD {
...
// When result.NewBalance is available (from repository transaction result), it is used directly
...
// When result.QuotaState is available (from repository transaction result), it is passed directly
...
func (s *GatewayService) buildCountTokensRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, mimicClaudeCode bool) (*http.Request, error) {
	...
	// Compute final anthropic-beta before body sanitization.
