...
type OpenAIRecordUsageInput struct {
	...
	// RequestType is the explicit usage classification selected by the handler.
	// Zero keeps the legacy stream/openai_ws_mode fallback behavior.
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// 官方 xAI Grok 视频计费已排除；保留参数兼容 calculateOpenAIRecordUsageCost 签名。
	...
	if input.RequestType.IsValid() && input.RequestType != RequestTypeUnknown {
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	// 官方 xAI Grok 视频计费（isGrokVideoUsageResult / calculateOpenAIVideoCost）已从本 fork 排除；
	// videoMultiplier 仍保留在签名中，避免调用方与上游接口漂移。
	_ = videoMultiplier
...
// 这些字段的旧快照或手工构造的上下文对象生成）。image 独立倍率在数据库中的
// 默认值为 1.0，正常加载的分组不可能倍率为 0 且未开启独立倍率、全部图片价为 nil——
