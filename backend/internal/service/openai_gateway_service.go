...
type OpenAIForwardResult struct {
	...
	FirstTokenMs          *int
	// UpstreamFirstEventMs records the first non-empty upstream SSE data event.
	// It remains separate from FirstTokenMs, which measures semantic output.
...
type OpenAIGatewayService struct {
	...
	openaiTransportBackoffMu            sync.Mutex
	openaiTransportBackoffByAccount     map[int64]openAITransportBackoffState
