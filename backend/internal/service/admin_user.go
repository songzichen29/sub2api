...
// PromoteUserToAdmin 将普通用户提升为管理员。已是管理员时幂等返回；
// 禁用用户不允许提升，避免绕过状态控制获得后台权限。
func (s *adminServiceImpl) PromoteUserToAdmin(ctx context.Context, id int64) (*User, error) {
	...
	logger.LegacyPrintf("service.admin", "audit: user promoted to admin target_user_id=%d", user.ID)
	return user, nil
}
