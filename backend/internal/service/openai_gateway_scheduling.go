...
func isOpenAICompatibleAccountEligibleForRequestBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	...
	if account == nil || account.Platform != platform || !account.IsOpenAI() || !account.IsSchedulableForModelWithContext(ctx, requestedModel) {
		return false
	}
	if account.IsOpenAI() {
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, error) {
	...
	// 1. 获取可调度的 OpenAI 账号。即使存在粘性绑定，也必须读取当前
	// 候选池，才能在管理员提升其他账号优先级后及时切换。
	...
	// 3. 粘性只在当前账号仍处于最高可用优先级层时生效。数值更小的
	// 优先级账号出现后，应覆盖旧绑定；同优先级仍保持会话连续性。
	sticky := s.tryStickySessionHit(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, requireCompact, stickyAccountID, requiredCapability)
	if sticky != nil && (selected == nil || selected.Priority >= sticky.Priority) {
		return sticky, nil
	}
...
func (s *OpenAIGatewayService) hasAvailableHigherPriorityOpenAIAccount(
	ctx context.Context,
	groupID *int64,
	platform string,
	accounts []Account,
	...
	for i := range accounts {
		...
		if !isOpenAICompatibleAccountEligibleForRequest(ctx, candidate, platform, requestedModel, requireCompact, requiredCapability) {
			continue
		}
			...
			if err == nil {
				...
				if !clearSticky && isOpenAICompatibleAccountEligibleForRequest(ctx, account, platform, requestedModel, false, requiredCapability) {
					...
					if account == nil {
						...
						if s.hasAvailableHigherPriorityOpenAIAccount(
							ctx,
							groupID,
							platform,
							accounts,
							...
							// Continue to Layer 2. It will select and bind an available
							// account from the higher-priority tier.
						} else {
...
func firstRequiredOpenAIEndpointCapability(requiredCapabilities []OpenAIEndpointCapability) OpenAIEndpointCapability {
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccountBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if s.isOAuthQuotaNearLimit(fresh) || s.isOpenAIProxyStreamQuarantined(ctx, fresh) {
		return nil
	}
	return fresh
}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if s.schedulerSnapshot == nil || s.accountRepo == nil {
		...
		if s.isOAuthQuotaNearLimit(account) || s.isOpenAIProxyStreamQuarantined(ctx, account) {
			return nil
		}
		return account
	}
	...
	if s.isOAuthQuotaNearLimit(latest) || s.isOpenAIProxyStreamQuarantined(ctx, latest) {
		return nil
	}
	return latest
}
