...
func (s *adminServiceImpl) ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode string, tags []string) ([]Account, error) {
	...
	return s.accountRepo.ListAllWithFilters(ctx, platform, accountType, status, search, groupID, privacyMode, tags)
}
...
func (s *adminServiceImpl) DuplicateAccount(ctx context.Context, id int64) (*Account, error) {
	...
	return s.CreateAccount(ctx, &CreateAccountInput{
		Name:                  buildDuplicateAccountName(src.Name),
		Notes:                 src.Notes,
...
// 保留 quota_limit / reset_mode / window_cost_limit / base_rpm 等配置字段，
// 仅删除用量计数、周期起点、限流计数和通知状态等运行态。
