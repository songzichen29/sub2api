...
// OllamaCloudUsageIsAutoRefreshDue exposes the canonical due calculation to
// database adapters that intentionally evaluate candidate timestamps in Go.
func OllamaCloudUsageIsAutoRefreshDue(snapshot *OllamaCloudUsageSnapshot, groupLastUsedAt *time.Time, now time.Time, debounce, maxWait time.Duration) bool {
	return ollamaCloudUsageIsAutoRefreshDue(snapshot, groupLastUsedAt, now, debounce, maxWait)
}
