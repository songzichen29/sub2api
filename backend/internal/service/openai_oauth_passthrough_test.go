...
func TestOpenAIGatewayService_OAuthPassthrough_StreamingSetsFirstTokenMs(t *testing.T) {
	...
	require.GreaterOrEqual(t, *result.UpstreamFirstEventMs, 0)
	upstreamFirstEventMs, ok := c.Get(OpsOpenAIUpstreamFirstEventMsKey)
	require.True(t, ok)
	require.Equal(t, int64(*result.UpstreamFirstEventMs), upstreamFirstEventMs)
	require.NotNil(t, result.ServiceTier)
