...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	// 分组未配置单价：默认 0.01。搜索按次使用不含高峰因子的基础倍率 2.0，
	// 即使 token 倍率为 3.0 也不采用。
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 2.0, UsageTokens{}, "")
