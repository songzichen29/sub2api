...
// IsAffiliateRechargeEnabled 检查是否启用充值返利。
// 旧库没有该设置时按默认值开启，保持原“邀请充值返利”兼容行为。
func (s *SettingService) IsAffiliateRechargeEnabled(ctx context.Context) bool {
	raw, err := s.settingRepo.GetValue(ctx, SettingKeyAffiliateRechargeEnabled)
	if err != nil {
...
// IsAffiliateSubscriptionEnabled 检查是否启用订阅返利。
func (s *SettingService) IsAffiliateSubscriptionEnabled(ctx context.Context) bool {
	raw, err := s.settingRepo.GetValue(ctx, SettingKeyAffiliateSubscriptionEnabled)
	if err != nil {
...
// GetAffiliateRechargeRebateRatePercent 读取充值返利比例。
// 未单独配置时回退到全局邀请返利比例，保持向后兼容。
func (s *SettingService) GetAffiliateRechargeRebateRatePercent(ctx context.Context) float64 {
	return s.getAffiliateKindRebateRatePercent(ctx, SettingKeyAffiliateRechargeRebateRate)
}

// GetAffiliateSubscriptionRebateRatePercent 读取订阅返利比例。
// 未单独配置时回退到全局邀请返利比例，保持向后兼容。
func (s *SettingService) GetAffiliateSubscriptionRebateRatePercent(ctx context.Context) float64 {
	return s.getAffiliateKindRebateRatePercent(ctx, SettingKeyAffiliateSubscriptionRebateRate)
}

func (s *SettingService) getAffiliateKindRebateRatePercent(ctx context.Context, key string) float64 {
