...
func TestOpenAIGatewayService_Forward_APIKeyNormalizesStringInput(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
	...
	body := []byte(`{"model":"gpt-5.4-mini","stream":false,"input":"hello from string input"}`)
	result, err := svc.Forward(context.Background(), c, account, body)
	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	...
	require.Equal(t, "hello from string input", gjson.GetBytes(upstream.lastBody, "input.0.content").String())
}

func TestOpenAIGatewayService_Forward_InjectsPriorityTierFromPolicy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
	...
	policySvc := newOpenAIGatewayServiceWithSettings(t, settings)
	svc := &OpenAIGatewayService{
		cfg:            cfg,
		httpUpstream:   upstream,
		settingService: policySvc.settingService,
	}
	account := &Account{
		ID:          3,
		Name:        "openai-apikey",
