...
func main() {
	...
	if *execute && deleted > 0 {
		fmt.Println("cleanup complete; schedule OPTIMIZE TABLE ops_error_logs during normal maintenance if space reclamation is needed")
	}
}
	...
	for {
		...
		if execute && len(ids) > 0 {
			...
			result, err := db.ExecContext(ctx,
				`DELETE FROM ops_error_logs WHERE id IN (`+placeholders+`) AND created_at < ?`, args...)
			if err != nil {
				return nil, scanned, matched, deleted, err
			}
